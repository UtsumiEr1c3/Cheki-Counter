...
Cheki Counter 是一个用 Flutter 编写的离线记账工具，用来记录偶像切写（cheki）相关数据，并提供活动视角、偶像视角和统计视角的汇总分析。
...
- 业务重点在“偶像 / 活动 / 切写记录”三类数据的组织与统计

## 功能概览

- 首页用卡片网格展示所有已有记录的偶像，并显示总切数、偶像数、总金额
- 支持为已有偶像追加切写记录，单价会默认带出该偶像最近一次记录的价格
- 支持新建偶像，同时要求录入第一条切写记录
- 支持新建活动，即使该活动暂时没有关联任何切写记录也可以单独保存
