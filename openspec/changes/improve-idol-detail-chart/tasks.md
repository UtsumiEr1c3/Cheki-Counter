...
- [ ] 3.2 在设备/模拟器上手动验证：选一个白色应援色偶像进详情页，确认按日和按月图的折线都清晰可见（非纯白）
- [ ] 3.3 在设备/模拟器上手动验证：选一个记录稀疏的偶像（同月内日期有跳跃）进详情页，切到按日模式，确认所有相邻点用直线连成一条连续折线
- [ ] 3.4 在设备/模拟器上手动验证：选一个深色应援色偶像（如红色/蓝色）进详情页，确认折线色仍为原应援色、未被降亮
